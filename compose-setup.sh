...
setup_database() {
  ...
  TIMEOUT=90
  COUNT=0
  printf "Artisan: configuring database..."
  docker-compose run --rm artisan php artisan migrate --seed --env=production &> /dev/null
...
clean() {
  echo "The setup will destroy the containers used by Artisan, removing also their volumes."
  while true; do
